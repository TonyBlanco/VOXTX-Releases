...
## PLATAFORMA: macOS — ESTADO: 🟡 En progreso (código implementado, pendiente build/firma)

### Implementado (rama `platform/macos`)
- `pubspec.yaml`: `media_kit_libs_macos_video: ^1.0.4` ✅
- `macos/Runner/DebugProfile.entitlements`: network.client + files.user-selected.read-write + JIT ✅
- `macos/Runner/Release.entitlements`: network.client + files.user-selected.read-write + JIT ✅
- `main.dart`: window_manager inicializado para macOS (1280×720, titleBarStyle.hidden, traffic lights nativos) ✅
- `main.dart`: WindowListener añadido/removido para macOS (graceful shutdown) ✅
- `lib/core/platform/macos_fullscreen_channel.dart`: fullscreen vía window_manager (sin canal nativo) ✅
- `platform_detector.dart`: `isMacOS` getter añadido ✅
- `dlna_service.dart`: device UUID + name vía macOsInfo.systemGUID / computerName ✅
- `update_service.dart`: fileName `VoXTV-macOS.dmg` para descargas en macOS ✅
