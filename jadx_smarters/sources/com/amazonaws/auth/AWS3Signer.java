...
public class AWS3Signer extends AbstractAWSSigner {
    ...
    public String getCanonicalizedHeadersForStringToSign(Request<?> request) {
        ...
        for (int i9 = 0; i9 < headersForStringToSign.size(); i9++) {
            headersForStringToSign.set(i9, StringUtils.lowerCase(headersForStringToSign.get(i9)));
        }
    ...
    public boolean shouldUseHttpsScheme(Request<?> request) {
        try {
            ...
            if (ClientConstants.DOMAIN_SCHEME.equals(strLowerCase)) {
                return true;
            }
            throw new AmazonClientException("Unknown request endpoint protocol encountered while signing request: " + strLowerCase);
        } catch (MalformedURLException e9) {
            throw new AmazonClientException("Unable to parse request endpoint during signing", e9);
        }
    }
