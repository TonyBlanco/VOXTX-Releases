...
public class KeyProvider18 implements KeyProvider {
    ...
    private byte[] rsaDecrypt(String str, byte[] bArr) {
        try {
            ...
            logger.error("Exception occurred while decrypting the encrypted AES key. ", e9);
            return null;
        }
    }
    ...
    private byte[] rsaEncrypt(String str, byte[] bArr) {
        try {
            ...
            logger.error("Exception occurred while encrypting data. " + e9.getMessage());
            return null;
        }
    }
    ...
    public synchronized void deleteKey(String str) {
        try {
            ...
            logger.error("Error in deleting the encrypted AES key identified by AesGcmNoPadding18-encrypted-encryption-key from SharedPreferences.", e9);
        }
        try {
            KeyStore keyStore = KeyStore.getInstance(ANDROID_KEY_STORE_NAME);
            keyStore.load(null);
            ...
            logger.error("Error in deleting the RSA Key identified by the keyAlias: " + str + " from " + ANDROID_KEY_STORE_NAME, e10);
        }
    }
    ...
    public synchronized Key generateKey(String str) throws KeyNotGeneratedException {
        ...
        try {
            ...
            try {
                ...
                throw new KeyNotGeneratedException("Error in generating the AES key and RSA encrypting the AES key using the rsaKeyAlias: " + str + " in " + ANDROID_KEY_STORE_NAME, e9);
            }
        } catch (Exception e10) {
            throw new KeyNotGeneratedException("Error in generating the RSA Encryption key for the rsaKeyAlias: " + str + " in " + ANDROID_KEY_STORE_NAME, e10);
        }
    ...
    public synchronized Key retrieveKey(String str) throws KeyNotFoundException {
        ...
        try {
            ...
            throw new KeyNotFoundException("Error occurred while accessing AndroidKeyStore to retrieve the key for keyAlias: " + str, e9);
        }
        return new SecretKeySpec(bArrRsaDecrypt, "AES");
    }
}
