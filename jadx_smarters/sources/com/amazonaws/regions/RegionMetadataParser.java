...
public class RegionMetadataParser {
    ...
    private static void addRegionEndpoint(Region region, Element element, boolean z9) {
        ...
        if (!z9 || verifyLegacyEndpoint(childElementValue2)) {
    ...
    private static List<Region> internalParse(InputStream inputStream, boolean z9) throws IOException {
        try {
            try {
                ...
                NodeList elementsByTagName = documentBuilderFactoryNewInstance.newDocumentBuilder().parse(inputStream).getElementsByTagName(REGION_TAG);
                ArrayList arrayList = new ArrayList();
                for (int i9 = 0; i9 < elementsByTagName.getLength(); i9++) {
                    Node nodeItem = elementsByTagName.item(i9);
                    if (nodeItem.getNodeType() == 1) {
                        arrayList.add(parseRegionElement((Element) nodeItem, z9));
                    }
                }
                return arrayList;
            } finally {
    ...
    private static Region parseRegionElement(Element element, boolean z9) {
        ...
        for (int i9 = 0; i9 < elementsByTagName.getLength(); i9++) {
            addRegionEndpoint(region, (Element) elementsByTagName.item(i9), z9);
        }
        return region;
    }

    private static boolean verifyLegacyEndpoint(String str) {
        return str.endsWith("TryRoom");
    }
    ...
    @Deprecated
    public List<Region> parseRegionMetadata(InputStream inputStream, boolean z9) throws IOException {
        return internalParse(inputStream, z9);
    }
}
