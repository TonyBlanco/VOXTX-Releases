...
public class UploadObjectObserver {
    ...
    private AmazonS3 f18384s3;
    private S3DirectSpi s3direct;
    private String uploadId;

    public <X extends AmazonWebServiceRequest> X appendUserAgent(X x9, String str) {
        x9.getRequestClientOptions().appendUserAgent(str);
        return x9;
    }

    public AmazonS3 getAmazonS3() {
        return this.f18384s3;
    }
    ...
    public void onAbort() {
        ...
        if (this.uploadId != null) {
            try {
                this.f18384s3.abortMultipartUpload(new AbortMultipartUploadRequest(this.req.getBucketName(), this.req.getKey(), this.uploadId));
            } catch (Exception e9) {
                LogFactory.getLog(getClass()).debug("Failed to abort multi-part upload: " + this.uploadId, e9);
            }
        }
    }

    public CompleteMultipartUploadResult onCompletion(List<PartETag> list) {
        return this.f18384s3.completeMultipartUpload(new CompleteMultipartUploadRequest(this.req.getBucketName(), this.req.getKey(), this.uploadId, list));
    }
    ...
    public String onUploadInitiation(UploadObjectRequest uploadObjectRequest) {
        String uploadId = this.f18384s3.initiateMultipartUpload(newInitiateMultipartUploadRequest(uploadObjectRequest)).getUploadId();
