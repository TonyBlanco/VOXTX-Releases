...
public final class zzaq implements zzap {
    ...
    private static final void zzb(byte[] bArr) {
        for (zzkx zzkxVar : zzla.zzi(bArr).zzj()) {
            if (k.h("INIT_TOTAL", "EXECUTE_TOTAL").contains(zzkxVar.zzj().name()) && zzkxVar.zzS()) {
                zzkxVar.zzH();
                zzkxVar.zzI();
                zzkxVar.zzj().name();
                zzkxVar.zzg().zzk();
    ...
    public final boolean zza(@NotNull byte[] bArr) {
        ...
        try {
            ...
            if (URLUtil.isHttpUrl(this.zza)) {
                ...
                l.c(uRLConnectionOpenConnection2, "null cannot be cast to non-null type javax.net.ssl.HttpsURLConnection");
