...
public class ISODateTimeFormat {
    ...
    public static final class Constants {
        ...
        private static final DateTimeFormatter f46157t = time();
        ...
        private static DateTimeFormatter time() {
            DateTimeFormatter dateTimeFormatter = f46157t;
    ...
    public static DateTimeFormatter forFields(Collection<DateTimeFieldType> collection, boolean z9, boolean z10) {
        ...
        if (hashSet.contains(DateTimeFieldType.monthOfYear())) {
            zDateByWeek = dateByMonth(dateTimeFormatterBuilder, hashSet, z9, z10);
            z11 = zDateByWeek;
        } else {
            if (hashSet.contains(DateTimeFieldType.dayOfYear())) {
                zDateByWeek = dateByOrdinal(dateTimeFormatterBuilder, hashSet, z9, z10);
            } else {
                    ...
                    if (!hashSet.contains(DateTimeFieldType.dayOfMonth())) {
                        if (!hashSet.contains(DateTimeFieldType.dayOfWeek())) {
                            if (hashSet.remove(DateTimeFieldType.year())) {
                                dateTimeFormatter = Constants.ye;
                            } else if (hashSet.remove(DateTimeFieldType.weekyear())) {
                                dateTimeFormatter = Constants.we;
                            } else {
                                z11 = false;
                            }
                ...
                zDateByWeek = dateByWeek(dateTimeFormatterBuilder, hashSet, z9, z10);
            }
            z11 = zDateByWeek;
        }
        time(dateTimeFormatterBuilder, hashSet, z9, z10, z11, hashSet.size() < size);
