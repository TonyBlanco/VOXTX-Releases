    ...
    public ImmutableHttpProcessor(HttpRequestInterceptorList httpRequestInterceptorList, HttpResponseInterceptorList httpResponseInterceptorList) {
        if (httpRequestInterceptorList != null) {
            ...
            for (int i9 = 0; i9 < requestInterceptorCount; i9++) {
                this.requestInterceptors[i9] = httpRequestInterceptorList.getRequestInterceptor(i9);
            }
        } else {
            this.requestInterceptors = new HttpRequestInterceptor[0];
        }
        ...
        for (int i10 = 0; i10 < responseInterceptorCount; i10++) {
            this.responseInterceptors[i10] = httpResponseInterceptorList.getResponseInterceptor(i10);
        }
    }
