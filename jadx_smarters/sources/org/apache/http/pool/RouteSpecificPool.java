...
abstract class RouteSpecificPool<T, C, E extends PoolEntry<T, C>> {
    ...
    public RouteSpecificPool(T t9) {
        this.route = t9;
    }

    public E add(C c9) {
        E e9 = (E) createEntry(c9);
        this.leased.add(e9);
        return e9;
    }

    public abstract E createEntry(C c9);

    public void free(E e9, boolean z9) {
        Args.notNull(e9, "Pool entry");
        Asserts.check(this.leased.remove(e9), "Entry %s has not been leased from this pool", e9);
        if (z9) {
    ...
    public boolean remove(E e9) {
        Args.notNull(e9, "Pool entry");
        return this.available.remove(e9) || this.leased.remove(e9);
    }
