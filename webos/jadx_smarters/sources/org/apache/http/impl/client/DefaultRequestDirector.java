...
public class DefaultRequestDirector implements RequestDirector {
    ...
    private void abortConnection() {
        ...
        if (managedClientConnection != null) {
            ...
            try {
                ...
                this.log.debug("Error releasing connection", e10);
            }
        }
    }
    ...
    private void tryConnect(RoutedRequest routedRequest, HttpContext httpContext) throws HttpException, IOException {
        ...
        while (true) {
            ...
            try {
                ...
                if (!this.retryHandler.retryRequest(e9, i9, httpContext)) {
                    throw e9;
                }
                if (this.log.isInfoEnabled()) {
                    this.log.info("I/O exception (" + e9.getClass().getName() + ") caught when connecting to " + route + ": " + e9.getMessage());
                    if (this.log.isDebugEnabled()) {
                        this.log.debug(e9.getMessage(), e9);
                    }
    ...
    private HttpResponse tryExecute(RoutedRequest routedRequest, HttpContext httpContext) throws HttpException, IOException {
        ...
        while (true) {
            ...
            try {
                ...
                if (!this.retryHandler.retryRequest(e9, request.getExecCount(), httpContext)) {
                ...
                if (this.log.isInfoEnabled()) {
                    this.log.info("I/O exception (" + e9.getClass().getName() + ") caught when processing request to " + route + ": " + e9.getMessage());
                }
                if (this.log.isDebugEnabled()) {
                    this.log.debug(e9.getMessage(), e9);
                }
    ...
    public boolean createTunnelToProxy(HttpRoute httpRoute, int i9, HttpContext httpContext) throws HttpException, IOException {
    ...
    public RoutedRequest handleResponse(RoutedRequest routedRequest, HttpResponse httpResponse, HttpContext httpContext) throws HttpException, IOException {
        ...
        int i9 = this.redirectCount;
        if (i9 >= this.maxRedirects) {
            throw new RedirectException("Maximum redirects (" + this.maxRedirects + ") exceeded");
        }
        this.redirectCount = i9 + 1;
    ...
    public void releaseConnection() {
        try {
            ...
            this.log.debug("IOException releasing connection", e9);
        }
        this.managedConn = null;
    }

    /* JADX WARN: Removed duplicated region for block: B:11:0x0022 A[Catch: URISyntaxException -> 0x0020, TryCatch #0 {URISyntaxException -> 0x0020, blocks: (B:2:0x0000, B:4:0x000b, B:6:0x0011, B:8:0x0017, B:15:0x0032, B:11:0x0022, B:12:0x0027, B:14:0x002d), top: B:19:0x0000 }] */
    /*
