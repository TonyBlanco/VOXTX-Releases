...
public class RefinedSoundex implements StringEncoder {
    ...
    public char getMappingCode(char c9) {
        if (Character.isLetter(c9)) {
            return this.soundexMapping[Character.toUpperCase(c9) - 'A'];
        }
        return (char) 0;
    }
    ...
    public String soundex(String str) {
        ...
        char c9 = '*';
        for (int i9 = 0; i9 < strClean.length(); i9++) {
            char mappingCode = getMappingCode(strClean.charAt(i9));
            if (mappingCode != c9) {
                if (mappingCode != 0) {
