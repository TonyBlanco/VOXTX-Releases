...
class TransferThreadPool {
    ...
    private static ExecutorService buildExecutor(int i9) {
        ThreadPoolExecutor threadPoolExecutor = new ThreadPoolExecutor(i9, i9, 10L, TimeUnit.SECONDS, new LinkedBlockingQueue());
    ...
    public static synchronized void init(int i9) {
        try {
            LOGGER.debug("Initializing the thread pool of size: " + i9);
            int iMax = Math.max((int) Math.ceil(((double) i9) / 2.0d), 1);
