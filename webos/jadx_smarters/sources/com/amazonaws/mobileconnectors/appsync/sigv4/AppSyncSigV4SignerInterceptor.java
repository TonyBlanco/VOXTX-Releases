    ...
    public Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        if (AuthMode.IAM.equals(this.authMode)) {
            try {
                ...
                throw new IOException("Failed to read credentials to sign the request.", e9);
            }
        } else if (AuthMode.API_KEY.equals(this.authMode)) {
            defaultRequest.addHeader(X_API_KEY, this.apiKey.getAPIKey());
            if (this.subscriberUUID != null) {
                Log.d(TAG, "Subscriber ID is " + this.subscriberUUID);
                defaultRequest.addHeader(X_AMZ_SUBSCRIBER_ID, this.subscriberUUID);
            }
            ...
            try {
                ...
                throw new IOException("Failed to retrieve AWS Lambda authorization token.", e12);
            }
        }
        ...
        builder.method(request.method(), RequestBody.create(JSON_MEDIA_TYPE, bufferClone.readByteArray()));
