    ...
    public AmazonServiceException handle(HttpResponse httpResponse) throws Exception {
        try {
            ...
            try {
                ...
                return newAmazonServiceException(String.format("Unable to unmarshall error response (%s)", string), httpResponse, e9);
            }
        } catch (IOException e10) {
            Log log2 = log;
            if (log2.isDebugEnabled()) {
                log2.debug("Failed in reading the error response", e10);
            }
            return newAmazonServiceException("Unable to unmarshall error response", httpResponse, e10);
        }
    }
