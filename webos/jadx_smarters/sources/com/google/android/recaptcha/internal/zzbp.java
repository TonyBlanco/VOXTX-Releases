package com.google.android.recaptcha.internal;

import java.lang.reflect.Array;
import java.util.List;
import org.jetbrains.annotations.NotNull;

/* JADX INFO: loaded from: classes3.dex */
public final class zzbp implements zzby {

    @NotNull
    public static final zzbp zza = new zzbp();

    private zzbp() {
    }

    @Override // com.google.android.recaptcha.internal.zzby
    public final void zza(int i9, @NotNull zzbl zzblVar, @NotNull Object... objArr) throws zzt {
        if (objArr.length != 2) {
            throw new zzt(4, 3, null);
        }
        Object obj = objArr[0];
        if (true != (obj instanceof Object)) {
            obj = null;
        }
        if (obj == null) {
            throw new zzt(4, 5, null);
        }
        Object obj2 = objArr[1];
        if (true != (obj2 instanceof Integer)) {
            obj2 = null;
        }
        Integer num = (Integer) obj2;
        if (num == null) {
            throw new zzt(4, 5, null);
        }
        int iIntValue = num.intValue();
        try {
            zzblVar.zzc().zzf(i9, obj instanceof String ? String.valueOf(((String) obj).charAt(iIntValue)) : obj instanceof List ? ((List) obj).get(iIntValue) : Array.get(obj, iIntValue));
        } catch (Exception e9) {
            if (!(e9 instanceof ArrayIndexOutOfBoundsException)) {
                throw new zzt(4, 23, e9);
            }
            throw new zzt(4, 22, e9);
        }
    }
}
