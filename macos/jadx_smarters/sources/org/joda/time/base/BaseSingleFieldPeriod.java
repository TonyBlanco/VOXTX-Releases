...
public abstract class BaseSingleFieldPeriod implements ReadablePeriod, Comparable<BaseSingleFieldPeriod>, Serializable {
    ...
    public static int standardPeriodIn(ReadablePeriod readablePeriod, long j9) {
        ...
        for (int i9 = 0; i9 < readablePeriod.size(); i9++) {
            ...
            if (value != 0) {
                DurationField field = readablePeriod.getFieldType(i9).getField(instanceUTC);
                if (!field.isPrecise()) {
        ...
        return FieldUtils.safeToInt(jSafeAdd / j9);
    }
