...
abstract class BasicGJChronology extends BasicChronology {
    ...
    static {
        ...
        int i9 = 0;
        while (i9 < 11) {
            j9 += ((long) MIN_DAYS_PER_MONTH_ARRAY[i9]) * TimeChart.DAY;
            int i10 = i9 + 1;
            MIN_TOTAL_MILLIS_BY_MONTH_ARRAY[i10] = j9;
            j10 += ((long) MAX_DAYS_PER_MONTH_ARRAY[i9]) * TimeChart.DAY;
            MAX_TOTAL_MILLIS_BY_MONTH_ARRAY[i10] = j10;
            i9 = i10;
        }
    }

    public BasicGJChronology(Chronology chronology, Object obj, int i9) {
        super(chronology, obj, i9);
    }

    @Override // org.joda.time.chrono.BasicChronology
    public int getDaysInMonthMax(int i9) {
        return MAX_DAYS_PER_MONTH_ARRAY[i9 - 1];
    }
    ...
    public int getDaysInYearMonth(int i9, int i10) {
        return isLeapYear(i9) ? MAX_DAYS_PER_MONTH_ARRAY[i10 - 1] : MIN_DAYS_PER_MONTH_ARRAY[i10 - 1];
    }
    ...
    */
    ...
    public long getTotalMillisByYearMonth(int i9, int i10) {
        return isLeapYear(i9) ? MAX_TOTAL_MILLIS_BY_MONTH_ARRAY[i10 - 1] : MIN_TOTAL_MILLIS_BY_MONTH_ARRAY[i10 - 1];
    }
    ...
    public long getYearDifference(long j9, long j10) {
        ...
        long yearMillis2 = j10 - getYearMillis(year2);
        if (yearMillis2 >= FEB_29) {
            if (isLeapYear(year2)) {
                if (!isLeapYear(year)) {
                    yearMillis2 -= TimeChart.DAY;
                }
            } else if (yearMillis >= FEB_29 && isLeapYear(year)) {
                yearMillis -= TimeChart.DAY;
            }
        }
        int i9 = year - year2;
    ...
    public boolean isLeapDay(long j9) {
        return dayOfMonth().get(j9) == 29 && monthOfYear().isLeap(j9);
    }

    @Override // org.joda.time.chrono.BasicChronology
    public long setYear(long j9, int i9) {
        int year = getYear(j9);
        int dayOfYear = getDayOfYear(j9, year);
